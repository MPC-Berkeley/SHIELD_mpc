...
def test(smpc_config,config,policy,device,policy_type,l1_dual_dim,ca_dual_dim,l1_num,pred_mode):
    #Initiate logger
    logdir = (
        policy_type
        + "_"
...
def main(smpc_config,config):
    ...
    if config['joint_dual_pred']:
        ...
        policy=RAID_NET_V2(raidnet_config,observation_dim, 2*observation_dim, l1_num+ca_num, config['N']-1, num_layers, hidden_dim, lambda_dim=l1_num,lambda_ubd = smpc_config['l1_lmbd'])
        device=th.device("cuda:0" if th.cuda.is_available() else "cpu")
        policy.to(device)
    else:
        pred_mode = ['both duals','tertiary','binary']
        l1_policy = RAID_NET_V2(raidnet_config,int(observation_dim/(smpc_config['num_tvs'])), observation_dim, l1_num, config['N']-1, smpc_config['num_tvs'], num_layers//2, hidden_dim//2,lambda_dim=l1_num, lambda_ubd=smpc_config['l1_lmbd'],  pred_mode=['l1','binary','binary'])
    ...
    #Single forward pass of RAIDNET
    batch_dim = 10
    test_tensor = th.randn(batch_dim, observation_dim).to(device)
    pdb.set_trace()
    l1_policy(test_tensor.view(batch_dim, smpc_config['num_tvs'],-1))
    ca_policy(test_tensor.view(batch_dim, smpc_config['num_tvs'],-1))

    policy_type = 'RAIDNET'
    pretrain = config['pretrain']
    checkpoint = None

    bc_learner = test(smpc_config,config,policy,device,policy_type,l1_dual_dim=l1_dual_dim,ca_dual_dim=ca_dual_dim,l1_num=l1_num,pred_mode=pred_mode)
