...
class SMPC():
    ...
    def _add_constraints_and_cost(self):
        ...
        cost += -4.*self.Q_cost*ca.sum1(nom_s) + 3.5*self.Q_cost*nom_z_diff.T@nom_z_diff# penalizes slow progress (was -1, 3.5)
        cost += self.R_cost*ca.diff(ca.vertcat(self.u_prev,h),1,0).T@ca.diff(ca.vertcat(self.u_prev,h),1,0) # penalizes large input rates
        if self.offline:
            self.lin_ineq_l1 = []
            self.ca_ineq = []
        ...
        for k in range(self.N_TV):
            for j in range(len(self.mode_map)):
                ...
                for t in range(1, self.N):  # position at time-step 1 not a function of decision variables 
                    ...
                    #slack cost
                    cost += 1e3*self.slack**2

                    #obca_lambd cost for PD hessian
                    cost += 1e-3*ca.vec(obca_lmbd[k][m][:,t-1]).T@ca.vec(obca_lmbd[k][m][:,t-1])
                    ...
                    if self.solver=="ipopt":
                        ...
                        if self.offline:
                            ...
                            # self.ca_ineq.append(z.T@z-y**2) #C-> 70 x vars_pol
                            # self.ca_ineq.append(-y)
                            ...
                            if len(self.l1_constr[k][m][t-1])==0:
                                ...
                                self.lin_ineq_l1+=[K[k][m][t,2*t:2*(t+1)] - self.gain_l1[k][m][t-1]]
                                self.lin_ineq_l1+=[-K[k][m][t,2*t:2*(t+1)] - self.gain_l1[k][m][t-1]]
                            ...
                            obca_constr = ca.vertcat(*[ca.sqrt(z.T@z + 1e-4)<=y, 0<=y])
                            ...
                            # l1_switch = ca.if_else(self.gain_keep[k][j][t-1], l1_constr, ca.DM.ones(*l1_constr.shape))
        ...
        if self.offline:
            ...
            self.F_fn = ca.Function('F_fn', [self.vars_pol4screening, self.params], [ca.jacobian(ca.vertcat(*self.canon_prob_fn['f_l_i_c'](self.vars_pol4screening,self.params)),self.vars_pol4screening)])
            ...
            self.C_fn = [ca.Function('C_fn', [self.vars_pol4screening,self.params], [ca.jacobian(ca.simplify(ca_constr), self.vars_pol4screening).sparsity()]) for ca_constr in self.canon_prob_fn['f_ca_i'](self.vars_pol4screening, self.params)]
    ...
    def update(self, update_dict):
        ...
        if not self.offline:
            if 'l1_duals' in update_dict.keys():
                ...
                self._test_update_gain_and_constr_keeps(*[update_dict[key] for key in ['l1_duals', 'ca_duals']])
    ...
    def _set_canon_form_mats(self):
        ...
        self.F, self.f = self.F_fn(ca.DM(*self.vars_pol4screening.shape),self.opti.value(self.params)).toarray(), self.opti.value(ca.vertcat(*self.canon_prob_fn['f_l_i_c'](ca.DM(*self.vars_pol4screening.shape),self.params)))


        self.L = self.L_fn(ca.DM(*self.vars_pol4screening.shape)).toarray()

        # self.C =  (ca.jacobian(ca_constr, self.vars_pol4screening) for ca_constr in canon_prob_fn['f_ca_i'](self.vars_pol4screening, self.params))
        self.C = ca.vertcat(*[self.C_fn[k](ca.DM.zeros(*self.vars_pol4screening.shape),self.opti.value(self.params)).toarray() for k in range(len(self.C_fn))])
        # self.C2 = ca.vertcat(*[self.C_fn[k](5*ca.DM.ones(*self.vars_pol4screening.shape),self.opti.value(self.params)).toarray() for k in range(len(self.C_fn))])
        self.c = ca.vertcat(*self.canon_prob_fn['f_ca_i'](ca.DM.zeros(*self.vars_pol4screening.shape), self.opti.value(self.params)))
    ...
    def _test_update_gain_and_constr_keeps(self, l1_duals=None, ca_duals=None):
        if l1_duals is None:
            ...
            ca_duals_iter = flatten(ca_duals)

            S1 = np.diag(ca_duals) #if mu_tilde=1, then mu!=0. elif mu_tilde=0, then mu = 0
            S2 = np.diag(1-l1_duals) #if g1_tilde=1, then g1=0 or g1=lmbd. elif g1_tilde=0, then 0 < g1 < lmbd
            rho1 = 0.1
            rho2 = 0.1

            #Recover feasible dual solutions
            mu, eta, g1 = self.solve_dual_approximation(self.Q, self.L, self.F, self.C, self.p, self.d, self.f, self.c, S1, S2, rho1, rho2)
                ...
                for t in range(self.N-1):
                    if l1_duals is not None:
                        if not (k,j,t) in vars_seen:
                            gain_keep = self._safe_screen(l1_duals[k][j][t][0], gap_radius= gap ,dual_type='l1_dual')
                        ...
                        constr_keep = self._safe_screen(ca_duals[k][m][t][0], gap_radius=gap ,dual_type='ca_dual')
                        if t <= 1: 
                            constr_keep = 1
    ...
    def solve_dual_approximation(self,Q, L, F, C, p, d, f, c, S1, S2, rho1, rho2):
        """
        ...
        pdb.set_trace()
        Q_inv = np.linalg.inv(Q) #Q is a spar
        pdb.set_trace()
        A_grad_mu = np.hstack([C @ Q_inv @ C.T + 2*rho1*S1.T @ S1, -C@Q_inv@F.T, -2*C@Q_inv@L.T])
        A_grad_eta = np.hstack([-F @ Q_inv @ C.T, F @ Q_inv @ F.T, 2*F @ Q_inv @ L.T])
        A_grad_g1 = np.hstack([-2*L @ Q_inv @ C.T, 2*L @ Q_inv @ F.T, 4*L @ Q_inv @ L.T + 2*rho2*S2.T @ S2])
        A = np.vstack([A_grad_mu, A_grad_eta, A_grad_g1])
        pdb.set_trace()
        b_grad_mu = c.reshape((-1,1)) + C @ Q_inv @(p.reshape((-1,1)) - self.l1_lmbd*L.T @np.ones((n_g1,1)))
        b_grad_eta = -f.reshape((-1,1)) - F @ Q_inv @(p.reshape((-1,1))- self.l1_lmbd*L.T @np.ones((n_g1,1)))
        b_grad_g1 = -2*L @ Q_inv @(p.reshape((-1,1))- self.l1_lmbd*L.T @np.ones((n_g1,1))) + self.l1_lmbd*rho2*S2.T @ np.ones((n_g1,1))
        pdb.set_trace()
        b = np.vstack([b_grad_mu, b_grad_eta, b_grad_g1])

        # Solve least squares with iterative method
        x = lsmr(A, b, atol=1e-4, btol=1e-4)[0]
        pdb.set_trace()
    ...
    def verify_solution(self,mu, eta, g1, Q, L, F, C):
        # Check KKT conditions
        primal_feas = (C @ mu + F @ eta + L @ (2*g1 - self.l1_lmbd)).max()
        dual_feas = (mu.min(), g1.min(), (g1 - self.l1_lmbd).max())
    ...
    def _compute_gap_radius(self,f_mu, f_nu, f_g):
        ...
        eigs = sla.eigvalsh(self.Q.toarray()+1e-5, subset_by_index=[0,self.Q.shape[0]-1])
        cal_C = ca.vertcat(*self.C)
        cal_c = ca.vertcat(*self.c)
        
        eta, sigma = eigs[-1]**(-1), eigs[0]**(-1)
        grad_d = np.zeros((cal_c.shape[0]+self.f.shape[0]+f_g.shape[0],1))
        grad_d += ca.vertcat(cal_C, -self.F, -2*self.L)@np.linalg.inv(self.Q.toarray())@(cal_C.T@f_mu - self.p-self.F.T@f_nu - self.L.T@(2*f_g -self.l1_lmbd*np.ones_like(f_g)))
        grad_d+= ca.vertcat(cal_c, self.f, np.zeros_like(f_g))
        duals = np.concatenate((f_mu, f_nu, f_g))
        proj_dual = np.clip(duals - grad_d, 0, np.inf)
        proj_g = np.clip(proj_dual[f_mu.shape[0]+f_nu.shape[0]:,:], 0, self.l1_lmbd)
        proj_dual[f_mu.shape[0]+f_nu.shape[0]:,:]=proj_g
